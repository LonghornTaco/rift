...
// We intentionally avoid any React client state — this page has exactly one job.
export default async function PopupCompletePage() {
  ...
  // JSON.stringify is safe here because the payload is server-controlled primitives.
  const script = `
    try {
      if (window.opener) {
